...
bbref_lineups = function(season) {
  ...
  season0 = as.numeric(substr(season,1,4))+1
  ...
  teams = "https://www.basketball-reference.com/leagues/NBA_{season0}_per_game.html" %>% 
    ...
    pluck(1) %>% 
    # drop_na() %>% 
    filter(!Team %in% c("Team","TOT","","2TM","3TM","4TM","5TM")) %>% 
    distinct(Team) %>% 
    arrange(Team) %>% 
    pull()
  ...
  n_lineups = n_lineups %>% 
    mutate(TEAM_ABBREVIATION = case_when(
      TEAM_ABBREVIATION=="BRK" ~ "BKN",
      TEAM_ABBREVIATION=="CHO" ~ "CHA",
  ...
  left_join(n_lineups,
    ...
    select(SEASON,TEAM_ID,TEAM_ABBREVIATION,TEAM_NAME,LINEUPS) %>% 
    return()
}

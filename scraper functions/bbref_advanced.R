...
bbref_advanced = function(season) {
  ...
  suppressWarnings(
    ...
    select(PLAYER_NAME=PLAYER,POS,AGE,MP,PER,FG3_RATE=X3P_AR,FTA_RATE=F_TR,OREB_PCT=ORB_PERCENT,DREB_PCT=DRB_PERCENT,AST_PCT=AST_PERCENT,
           STL_PCT=STL_PERCENT,BLK_PCT=BLK_PERCENT,TOV_PCT=TOV_PERCENT,USG_PCT=USG_PERCENT,PER,OWS:WS_48,OBPM:VORP) %>% 
    return()
}
